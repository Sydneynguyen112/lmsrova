// Bộ câu hỏi khám bệnh dựng sẵn — dùng khi CHƯA có form intake published
// (soạn từ rova-ops). Đảm bảo luồng onboarding không bao giờ chết:
// bỏ qua bài test = onboarding_survey null = học viên kẹt stage 1 trong mọi analytics.
// Chạy pipeline submit y hệt form thật, chỉ khác form_id = null.
import type { IntakeQuestion } from "./intake-scoring";

let seq = 0;
function q(partial: Omit<IntakeQuestion, "form_id" | "correct_option" | "points" | "order_index">): IntakeQuestion {
  seq += 10;
  return {
    form_id: "",
    correct_option: null,
